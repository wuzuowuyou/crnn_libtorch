...
torch::Tensor pre_img(const cv::Mat &img, std::vector<float>v_mean, std::vector<float>v_std, int standard_w, int standard_h)
{
    cv::Mat m_stand;
    float ratio = 10.0;
    if(1 == img.channels()) { cv::cvtColor(img,img,CV_GRAY2BGR); }
    LstmImgStandardization(img, ratio,  standard_w,  standard_h, m_stand);
...
string get_label(const torch::Tensor &output)
{
//    torch::Tensor output = out_.squeeze(1);
...
int main()
{
    std::string path_pt = "./model0.9833.pt";
    std::string path_img_dir = "./data/test";
    ...
    int batch_size = 128;
    vector<torch::Tensor> v_input;
    vector<cv::Mat> v_img;

    for(int i=0;i<v_path.size();i+=batch_size)
    {
        cnt_all += 1;
        v_input.clear();
        v_img.clear();
        ...
        for(int j=0;j<batch_size;j++)
        {
            ...
            std::string path_img = v_path[i+j];
            std::cout<<i+j<<"  path="<<path_img<<std::endl;
        ...
        torch::Tensor input = torch::cat(v_input,0);
        ...
        for(int j=0;j<batch_size;j++)
        {
            std::string rec = get_label(output[j]);

            std::cout<<"rec="<<rec<<std::endl;
            cv::imshow("img",v_img[j]);
            cv::waitKey(0);
        }
